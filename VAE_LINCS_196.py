...
img_rows, img_cols, img_chns = 196, 196, 3 # mnist
#img_rows, img_cols, img_chns = 128, 128, 3 # mnist
...
#mat = hdf5storage.loadmat('LINCS_mat_128.mat')
mat = hdf5storage.loadmat('LINCS_mat_196.mat')
#mat = loadmat('LINCS_mat.mat')
x_train = mat['x_train']
y_train = mat['y_train']
x_test = mat['x_test']
y_test = mat['y_test']

id = np.random.permutation(x_test.shape[0])

x_test = x_test[id[0:2000],:,:,:]
y_test = y_test[id[0:2000]]
...
plt.colorbar()
plt.savefig('I_test_encoded_LINCS_196.png')
plt.close(fig)
...
plt.colorbar()
plt.savefig('I_train_encoded_LINCS_196.png')
plt.close(fig)




#scipy.io.savemat('./result_VAE_LINCS_128.mat', mdict={'x_test_encoded': x_test_encoded, 'y_test': y_test,
scipy.io.savemat('./result_VAE_LINCS_196.mat', mdict={'x_test_encoded': x_test_encoded, 'y_test': y_test,
...
for i in range(n):
	for j in range(n):
		...
		figure[ i* img_size: (i+1)*img_size, j*img_size:(j+1)*img_size] = img2
...
plt.imshow(figure)
plt.savefig('I_latent2_LINCS.png')
plt.close(fig)
#plt.show()
