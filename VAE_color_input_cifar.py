...
img_rows, img_cols, img_chns = 32, 32, 3 # mnist
...
x_train = np.empty((train_generator.n, img_rows, img_cols, img_chns))
y_train = train_generator.classes
x_test = np.empty((test_generator.n, img_rows, img_cols, img_chns))
y_test = test_generator.classes

for k in range(train_generator.n):
	im = imageio.imread(train_generator.directory+ '/'+train_generator.filenames[k])
	#im.transpose(1, 2, 0)
	img = im.reshape([-1, img_rows, img_cols, img_chns])
	x_train[k] = img

for k in range(test_generator.n):
	im = imageio.imread(test_generator.directory + '/' + test_generator.filenames[k])
	#im.transpose(1,2,0)
	img = im.reshape([-1, img_rows, img_cols, img_chns])
	x_test[k] = img
...
n = 30 #15  # figure with 15x15 digits
img_size = 32
